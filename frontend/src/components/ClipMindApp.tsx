...
import FeatureControlPanel, { FeatureValue } from './FeatureControlPanel';
...
const ClipMindApp = () => {
  ...
  // Feature Controls
  const [controls, setControls] = useState<Record<string, FeatureValue>>({
    'Auto captions': 'no-idea',
    '9:16 (TikTok)': 'no-idea',
    'AI Voice Male': 'no-idea'
  });
  ...
  const [decisions, setDecisions] = useState([
    { id: '1', icon: 'ti-subtitles', label: 'Synced bold captions for speech', status: 'pending', previewColor: '#00D4FF', previewText: 'TXT' },
    { id: '2', icon: 'ti-wave-sine', label: 'Dynamic glitch transitions', status: 'pending' },
    { id: '3', icon: 'ti-headphones', label: 'Energetic electronic background music', status: 'pending', previewText: 'MP3' },
    { id: '4', icon: 'ti-wand', label: 'Subtle vignette and film grain', status: 'pending' },
    { id: '5', icon: 'ti-crop', label: '9:16 Format for TikTok/Reels', status: 'pending', previewText: '9:16' },
  ]);
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.code === 'ArrowRight') setPlayheadPos(p => Math.min(100, p + 1));
      if (e.code === 'ArrowLeft') setPlayheadPos(p => Math.max(0, p - 1));
  ...
  const handleApplyAll = () => {
    setDecisions(prev => prev.map(d => d.status === 'pending' ? { ...d, status: 'selected' } : d));
  };

  const handleRejectAll = () => {
    setDecisions(prev => prev.map(d => d.status === 'pending' ? { ...d, status: 'skipped' } : d));
  };
  ...
  const toolGroups = [
    { label: 'Media', items: [
      { id: 'upload', icon: 'ti-upload', label: 'Upload Video' },
      { id: 'music-upload', icon: 'ti-music', label: 'Upload Audio' },
      { id: 'photo', icon: 'ti-photo', label: 'Upload Image' },
      { id: 'world', icon: 'ti-world', label: 'Stock Media', badge: 'SOON' }
    ] },
    { label: 'AI Tools', items: [
      { id: 'ai', icon: 'ti-sparkles', label: 'AI Generate' },
      { id: 'captions', icon: 'ti-subtitles', label: 'Auto Captions' },
      { id: 'narration', icon: 'ti-eye', label: 'Visual Narration' },
      { id: 'voice', icon: 'ti-microphone', label: 'Add Voice' },
      { id: 'translate', icon: 'ti-language', label: 'Translate', badge: 'SOON' }
    ] },
    { label: 'Edit', items: [
      { id: 'cut', icon: 'ti-cut', label: 'Split Clip' },
      { id: 'trim', icon: 'ti-arrows-horizontal', label: 'Trim' },
      { id: 'speed', icon: 'ti-gauge', label: 'Speed' },
      { id: 'crop', icon: 'ti-crop', label: 'Crop' },
      { id: 'rotate', icon: 'ti-rotate', label: 'Rotate' },
      { id: 'flip', icon: 'ti-flip-horizontal', label: 'Flip' },
      { id: 'trash', icon: 'ti-trash', label: 'Delete' },
      { id: 'copy', icon: 'ti-copy', label: 'Duplicate' }
    ] },
    { label: 'Visuals', items: [
      { id: 'filters', icon: 'ti-adjustments', label: 'Filters' },
      { id: 'color', icon: 'ti-palette', label: 'Color Grade' },
      { id: 'transitions', icon: 'ti-wave-sine', label: 'Transitions' },
      { id: 'effects', icon: 'ti-wand', label: 'Effects' },
      { id: 'layers', icon: 'ti-layers', label: 'Overlay' },
      { id: 'layout', icon: 'ti-layout', label: 'Background' }
    ] },
    { label: 'Text', items: [
      { id: 'text', icon: 'ti-text-size', label: 'Add Text' },
      { id: 'caption-style', icon: 'ti-text', label: 'Captions Style' },
      { id: 'sticker', icon: 'ti-mood-happy', label: 'Stickers' },
      { id: 'shape', icon: 'ti-square', label: 'Shapes' }
    ] },
    { label: 'Audio', items: [
      { id: 'music', icon: 'ti-headphones', label: 'Music' },
      { id: 'volume', icon: 'ti-volume', label: 'Volume' },
      { id: 'equalizer', icon: 'ti-chart-bar', label: 'Equalizer' },
      { id: 'noise', icon: 'ti-ear-off', label: 'Remove Noise' },
      { id: 'beat', icon: 'ti-wave-square', label: 'Beat Sync' }
    ] },
    { label: 'Format', items: [
      { id: 'aspect', icon: 'ti-aspect-ratio', label: 'Aspect Ratio' },
      { id: 'zoom', icon: 'ti-zoom-in', label: 'Zoom & Pan' },
      { id: 'watermark', icon: 'ti-droplet', label: 'Watermark' },
      { id: 'download', icon: 'ti-download', label: 'Export Settings' }
    ] }
  ];
  ...
  return (
    <div className={`h-screen w-screen bg-[#0E0E10] flex flex-col overflow-hidden text-white relative ${isFullscreen ? "cursor-none" : ""}`}>
      ...
      {showHelp && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[300] flex items-center justify-center p-6 animate-in zoom-in duration-300">
           <div className="bg-[#1E1E24] border border-white/10 w-full max-w-md rounded-3xl p-8 space-y-6 shadow-2xl">
              <h2 className="text-xl font-black flex items-center gap-3 uppercase tracking-tighter"><i className="ti ti-help-circle text-[#7C5CFC]"></i> Shortcuts & Guide</h2>
      ...
      {/* Zone 1: Top Bar */}
      {!isFullscreen && (
        <header className="h-14 bg-[#0E0E10] border-b border-white/5 flex items-center justify-between px-6 shrink-0 z-50 transition-standard">
          <div className="flex items-center gap-3">
            ...
            <span className="font-black text-lg tracking-tighter uppercase tracking-tighter">ClipMind</span>
          </div>
          ...
          <div className="flex-1 flex justify-center px-12">
             <input
               ...
               className="bg-transparent text-center text-sm font-bold tracking-tight focus:bg-white/5 px-4 py-1 rounded-xl outline-none transition-standard border border-transparent focus:border-white/10 w-full max-w-sm uppercase"
             />
          </div>
      ...
      <div className="flex-1 flex overflow-hidden relative">
        {/* Zone 2: Left Sidebar */}
        {!isFullscreen && (
          <aside className={`${sidebarCollapsed ? "w-0" : "w-[72px]"} bg-[#0E0E10] border-r border-white/5 flex flex-col py-6 shrink-0 overflow-y-auto custom-scrollbar no-scrollbar transition-standard`}>
        ...
        <div className="flex-1 flex flex-col overflow-hidden relative">
          <div className="flex-1 flex overflow-hidden">
            ...
            {activePanel && !isFullscreen && (
              <div className="w-[300px] border-r border-white/5 bg-[#0E0E10] shrink-0 p-6 overflow-y-auto custom-scrollbar animate-in slide-in-from-left duration-200">
                {activePanel === 'voice' ? <VoicePanel /> :
                 activePanel === 'upload' ? <MediaPanel /> :
                 activePanel === 'aspect' ? <FormatPanel /> : (
                  <div className="flex flex-col items-center justify-center h-full text-center space-y-4 opacity-40 transition-standard">
            ...
            <div className={`flex-1 bg-[#0E0E10] flex flex-col p-8 items-center justify-center relative min-w-0 transition-standard ${isFullscreen ? "p-0" : ""}`}>
               {error && (
                 <div className="absolute top-8 left-1/2 -translate-x-1/2 bg-red-500/10 border border-red-500/20 px-6 py-3 rounded-2xl flex items-center gap-4 z-50 animate-in fade-in slide-in-from-top-4 duration-300">
                    ...
                    <button onClick={() => handleGenerate("Retry")} className="bg-red-500 text-white px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest shadow-glow-purple">Retry</button>
                 </div>
               )}

               <div className={`w-full h-full max-w-4xl bg-black rounded-[40px] border border-white/5 shadow-2xl flex items-center justify-center relative group overflow-hidden transition-standard ${isFullscreen ? "max-w-none rounded-none border-0" : ""}`}>
                  <div className="absolute inset-0 flex items-center justify-center opacity-10">
                     <i className="ti ti-player-play text-zinc-100 text-[160px]"></i>
                  </div>

                  {/* Playback Controls */}
                  <div className="absolute bottom-8 left-1/2 -translate-x-1/2 bg-[#1E1E24]/80 backdrop-blur-xl px-8 py-4 rounded-3xl flex items-center gap-8 border border-white/10 shadow-2xl opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-4 group-hover:translate-y-0 z-20">
               ...
               {!isFullscreen && (
                 <div className="mt-6 flex items-center gap-3 text-[10px] font-black text-zinc-600 uppercase tracking-[0.2em] bg-[#1E1E24] px-4 py-2 rounded-full border border-white/5 shrink-0 transition-standard">
            ...
            {!isFullscreen && (
              <aside className={`${rightPanelCollapsed ? "w-0" : "w-[340px]"} bg-[#0E0E10] border-l border-white/5 overflow-y-auto custom-scrollbar flex flex-col shrink-0 transition-standard relative`}>
                 <button
                   ...
                   className="absolute left-0 top-1/2 -translate-y-1/2 w-4 h-12 bg-[#1E1E24] border border-white/5 rounded-l-lg flex items-center justify-center text-zinc-600 hover:text-white z-40 transition-standard shadow-2xl"
                 >
                   <i className={`ti ${rightPanelCollapsed ? 'ti-chevron-left' : 'ti-chevron-right'} text-xs`}></i>
                 </button>
                 {!rightPanelCollapsed && (
                   <div className="p-6 h-full transition-standard space-y-8 pb-32">
                      <div className="flex gap-2">
                        {(['auto', 'assist', 'manual'] as const).map(m => (
                          <button
                            key={m}
                            onClick={() => setMode(m)}
                            className={`flex-1 py-2 text-[10px] font-black uppercase tracking-widest rounded-xl border transition-standard ${mode === m ? 'bg-[#7C5CFC] border-[#7C5CFC] text-white' : 'bg-transparent border-white/5 text-zinc-500 hover:text-white hover:border-white/10'}`}
                          >
                            {m}
                          </button>
                        ))}
          ...
          {!isFullscreen && (
            <div className={`${timelineCollapsed ? "h-14" : "h-[340px]"} bg-[#1A1A1F] border-t border-white/5 flex flex-col shrink-0 relative overflow-hidden transition-standard`}>
                  ...
                  {!timelineCollapsed && (
                    <div className="flex items-center gap-6">
                       <div className="flex items-center gap-3 text-zinc-500">
                          ...
                          <span className="text-[10px] font-mono w-10 text-center">100%</span>
                       </div>
                    </div>
                  )}
               </div>
               ...
               {!timelineCollapsed && (
                 <div className="flex-1 flex flex-col p-8 min-h-0 bg-black/20 transition-standard overflow-hidden">
...
const ShortcutRow = ({ label, keybind }: any) => (
  <div className="flex justify-between items-center bg-zinc-950 p-3 rounded-xl border border-white/5 group hover:border-white/10 transition-standard">
    <span className="text-xs font-bold text-zinc-400 group-hover:text-zinc-200 uppercase tracking-tighter">{label}</span>
    <span className="px-2 py-1 bg-[#7C5CFC]/10 text-[#7C5CFC] text-[9px] font-black rounded-lg border border-[#7C5CFC]/20 group-hover:border-[#7C5CFC]/40 transition-standard">{keybind}</span>
  </div>
);
